...
stemmer = TurkishStemmer()#tr_specific
import requests
headers_tr = {'cache-control': 'no-cache',
...
with open("unigram_pos_lemma_tr.json","r") as f:
...
def look_for_similar (offset, interval, debug = DEBUG):
    ...
    request ="""SELECT word_id, ref_id,setting_id AS translation_id,
                ...
                WHERE (array_length(regexp_split_to_array(jdesc ->>'tr', '[ ''-]'), 1) = 1) AND (ref_id = 2) AND (word_id >= """ + str(offset) + """) AND (word_rating > 35)
