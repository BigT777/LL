...
START_WORD_ID = 123443
FIN_WORD_ID = 10000000
ngramm_len = 2
DEBUG = False
save_location = './save_results_bigramm_rus/'
...
def look_for_similar (word_id_offset, interval, similar_ngramms_list, debug = DEBUG):
    ...
    request ="""SELECT word_id, ref_id,setting_id AS translation_id,
                ...
                WHERE (array_length(regexp_split_to_array(jdesc ->>'tr', '[ ''-]'), 1) =""" + str(ngramm_len) + """) AND (ref_id = 1) AND (word_id >= """ + str(word_id_offset) + """) AND (word_rating > 100)
