...
#rus_specific
NGRAMMS_PATH = "trigramms_top.json"
NGRAMMS_ALPHABET_PATH = "trigramms_dict.json" 
START_WORD_ID = 234563
FIN_WORD_ID = 5000000#-100000 102396352
ngramm_len = 3
DEBUG = False
save_location = './save_results_trigramm_rus'
...
def look_for_similar (word_id_offset, interval, similar_ngramms_list, debug = DEBUG):
    ...
    request ="""SELECT word_id, ref_id,setting_id AS translation_id,
                ...
                WHERE (array_length(regexp_split_to_array(jdesc ->>'tr', '[ ''-]'), 1) =""" + str(ngramm_len) + """) AND (ref_id = 1) AND (word_id >= """ + str(word_id_offset) + """) AND (word_rating > 10)
            ...
            for first_letters in first_letters_list:
                ...
                if (first_letters in NGRAMM_ALPHA_DICT):
                    ...
                    for similar_ngramm_index in range(len(compare_list)):
                        ...
                        eng_detection = re.findall("[a-zA-Z0-9]",orig_line)#rus_specific
                        if len(eng_detection == 0):
